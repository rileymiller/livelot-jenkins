...
privKeyText = new File('/secure/gitlab-ssh-key.priv').text.trim()
passPhraseText = new File('/secure/gitlab-ssh-key.pw').text.trim()
sshUserText = new File('/secure/gitlab-ssh-key.user').text.trim()
...
desc.setGlobalConfigEmail("${sshUserText}@livelot.com")

// Save the descriptor
desc.save()

// Echo out (or log if you like) that we've done something
println("INFO: GitLab Credentials and Configuration complete!")
